...
const useStyles = makeStyles<{ open: boolean; isVerySmall: boolean }>()((theme, { open, isVerySmall }) => ({
    menuItem: {
    ...
    outOfMenuPaper: {
        ...
        transform: open ? `translate(${isVerySmall ? 0 : -20}px, -110px)` : "none",
        zIndex: open ? 2001 : 1,
        position: "absolute",
...
const FieldTypeMenuItem = forwardRef<HTMLLIElement, FieldTypeMenuItemArgs>(
    ({ field, onClick, inMenu = false, options = null }, ref) => {
        ...
        const menuItem = (
            <MenuItem ref={ref} onClick={onClick} className={classes.menuItem} disabled={open}>
                        ...
                        {options && (
                            <>
                                <Grid item className={classes.floatingOptionsHeading}>
                                    <Typography variant="subtitle1" className={classes.floatingOptionsHeadingText}>
                                        Options
                                    </Typography>
                                </Grid>
