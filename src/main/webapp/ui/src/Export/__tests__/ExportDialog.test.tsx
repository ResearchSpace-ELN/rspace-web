...
describe("ExportDialog", () => {
    ...
    describe("Validations should be enforced.", () => {
        describe("Exporting with filestores links", () => {
            test("but without being logged in should show a warning.", async () => {
                ...
                mockAxios.onPost("/nfsExport/ajax/createQuickExportPlan").reply(200, { ...CREATE_QUICK_EXPORT_PLAN });
                ...
                await waitFor(() => {
                    expect(
                        screen.getByText("Exported content contains 1 filestore link from 1 File System."),
                    ).toBeVisible();
                });
            ...
            test("but without scanning should show a warning.", async () => {
                ...
                await waitFor(() => {
                    expect(
                        screen.getByText("You are logged into all File Systems referenced by filestore links."),
                    ).toBeVisible();
                });
    ...
    describe("Controlled vocabulary terms", () => {
        ...
        test("Tags should be pre-populated from the tags on the documents", async () => {
            ...
            fireEvent.click(screen.getByRole("checkbox", { name: /Export to a repository/ }));
            ...
            await fc.assert(
                fc
                    .asyncProperty(fc.boolean(), async (setAllVersionsSwitch) => {
                        ...
                        expect(JSON.parse(mockAxios.history.post[0].data).exportConfig.allVersions).toBe(
                            setAllVersionsSwitch,
                        );
                ...
                fc.assert(
                    fc
                        .asyncProperty(
                            ...
                            async (selection) => {
                                mockAxios.onGet("/export/ajax/defaultPDFConfig").reply(200, { data: { pageSize } });
                                ...
                                await user.click(screen.getByRole("button", { name: "Next" }));
                                await waitFor(() => {
                                    expect(screen.getByRole("button", { name: pageSize })).toBeVisible();
                                });
                            },
                        )
                        .afterEach(cleanup),
