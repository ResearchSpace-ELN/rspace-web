var tinymcesetup = {
  ...
  menu: {
    file: {
      title: "File",
      items:
        "optSave optSaves | print | undo redo | searchreplace selectall | optSnip | confShortcuts confToolbar",
    },
    insert: {
      title: "Insert",
      items:
        "optMediaGallery optFromComputer | anchor charmap nonbreaking insertdatetime hr | link optInternallink optComments optSketch optMathjax codesample",
    },
    format: {
      title: "Format",
      items:
        "bold italic underline strikethrough superscript subscript | formats | indent outdent | removeformat",
    },
    table: {
      title: "Table",
      items:
        "inserttable inserttabledialog optHandsontable tableprops deletetable | cell row column",
    },
    view: {
      title: "View",
      items:
        "visualaid visualblocks visualchars optHideShowToolbar | code preview fullscreen",
    },
    tools: {
      title: "Science Tools",
  ...
  external_plugins: {
    confirmcancel:
      "/scripts/externalTinymcePlugins/confirmcancel/plugin.min.js",
    ...
    contexttoolbars:
      "/scripts/externalTinymcePlugins/contexttoolbars/plugin.min.js",
  ...
  content_css: [
    ...
    "/scripts/tinymce/tinymce516/plugins/codesample/css/prism.css",
  ],
  image_advtab: true,
  smart_paste: false,
  branding: false,
  ...
  paste_postprocess(plugin, args) {
    ...
    var processed = RS.tinymcePasteHandler.processPastedContent(
      innerText,
      innerHTML,
    );
    if (processed) {
  ...
  setup(ed) {
    ...
    ed.on("dblclick", function (e) {
      ...
      if ($target.is("img")) {
        if ($target.hasClass("commentIcon")) {
          ...
          $target.hasClass("attachmentIcon") &&
          $target.parents(".boxVersionLink").length
        ) {
          showBoxLinkInfo($target.parents(".boxVersionLink"));
        }
      } else if (
        $target.is("a") &&
        $target.hasClass("rsEquationClickableWrapper")
      ) {
        tinyMCE.activeEditor.execCommand("cmdRsMathJax");
      } else if (
        $target.hasClass("master-mix") ||
        $target.closest(".master-mix").size() > 0
      ) {
        tinyMCE.activeEditor.execCommand("cmdRSMasterMix");
      }

      var $targetAttachmentDivArea = $target.is(attachmentSelector)
        ? $target
        : $target.parents(attachmentSelector);
      ...
      var $targetCalcTableArea = $target.is(calcTableSelector)
        ? $target
        : $target.parents(calcTableSelector);
    ...
    ed.on("click", function (e) {
      ...
      if ($target.is("img")) {
        if ($target.hasClass("commentIcon")) {
          showFieldHint(
            $fieldName,
            "Hint: double-click on the comment to see the details",
          );
        } else if (
          $target.hasClass("imageDropped") ||
          $target.hasClass("sketch")
        ) {
          showFieldHint(
            $fieldName,
            "Hint: double-click on the image to open annotation tool",
          );
        } else if ($target.hasClass("chem")) {
          showFieldHint(
            $fieldName,
            "Hint: double-click on the chemical structure to edit",
          );
        }
      } else if (
        $target.is("a") &&
        ...
        showFieldHint(
          $fieldName,
          "Hint: double-click on the math equation to edit",
        );
      }
      ...
      if ($targetAttachmentDivArea.length) {
        showFieldHint(
          $fieldName,
          "Hint: double-click on the attachment to see the details",
        );
      }

      var $targetCodeSampleArea = $target.is(codeSampleAreaSelector)
        ? $target
        : $target.parents(codeSampleAreaSelector);
      ...
      if ($targetCalcTableArea.length) {
        showFieldHint(
          $fieldName,
          "Hint: double-click on calculation table to edit",
        );
      }
    });
    ...
    ed.on("keydown", function (e) {
      if (
        ...
        if ($currentNode.is("p")) {
          var contentWithoutSpaces = $currentNode
            .html()
            .replace(/&nbsp;/g, "")
            .replace(/<br>/g, "")
            .replace(/\s/g, "");
          ...
          if (
            ...
            contentWithoutSpaces === '<brdata-mce-bogus="1">'
          ) {
            $currentNode.remove();
            e.preventDefault();
            e.stopPropagation();
    ...
    ed.on("init", function () {
      ...
      var event = new CustomEvent("tinymce-iframe-loaded", {
        detail: `#${ed.id}_ifr`,
      });
  ...
  mentions: {
    ...
    source(query, process, delimiter) {
      let id = $(".rs-global-id a:not(.recordInfoIcon)")
        .text()
        .replace(/\D/g, "");
      var get_users = $.get(
        "/messaging/ajax/recipients",
        {
          term: query,
          messageType: "SIMPLE_MESSAGE",
          targetFinderPolicy: "STRICT",
          recordId: id,
        },
        function (data) {
          data = data.data;
    ...
    insert(item) {
      ...
      var optionalMessage =
        "&optionalMessage=You were mentioned in " +
        name +
        " ( " +
        global_id +
        " ).";
      ...
      return (
        '<span style="color: #1465b7"> + ' + item.fullname + " </span>&nbsp;"
      );
    },
  },
};
...
tinymce.PluginManager.add("commandpalette", function (editor) {
  ...
  editor.ui.registry.addAutocompleter("insertActions", {
    ...
    fetch: function (pattern) {
      const matchedActions = [...window.insertActions.values()]
        .filter((action) => {
          ...
          return (
            ...
            action.aliases?.some(
              (alias) =>
                alias.toLowerCase().indexOf(pattern.toLowerCase()) !== -1,
            )
          );
        })
...
function initTinyMCE(selector) {
  ...
  if (localStorage.getItem("custom_toolbar")) {
    let config = Object.values(
      JSON.parse(localStorage.getItem("custom_toolbar")),
    );
    toolbarRequest = $.Deferred();
    toolbarRequest.resolve(config);
  } else {
    toolbarRequest = $.getJSON(
      "/scripts/externalTinymcePlugins/toolbar/config.json",
    );
  }
  ...
  let requestsPromise = $.when(
    propertiesRequest,
    integrationsRequest,
    toolbarRequest,
  );
  requestsPromise.done(
    function (propertiesResponse, integrationsResponse, toolbarResponse) {
      ...
      var dropboxEnabled =
        ...
        integrations.DROPBOX.available &&
        integrations.DROPBOX.options["dropbox.linking.enabled"];
      var boxEnabled =
        integrations.BOX.enabled &&
        integrations.BOX.available &&
        integrations.BOX.options["box.linking.enabled"];
      var oneDriveEnabled =
        integrations.ONEDRIVE.enabled &&
        integrations.ONEDRIVE.available &&
        integrations.ONEDRIVE.options["onedrive.linking.enabled"];
      var googleDriveEnabled =
        integrations.GOOGLEDRIVE.enabled &&
        integrations.GOOGLEDRIVE.available &&
        integrations.GOOGLEDRIVE.options["googledrive.linking.enabled"];
      var egnyteEnabled =
        integrations.EGNYTE.enabled && integrations.EGNYTE.available;
      var gitHubEnabled =
        integrations.GITHUB.enabled && integrations.GITHUB.available;
      var chemistryEnabled =
        integrations.CHEMISTRY.enabled && integrations.CHEMISTRY.available;
      var protocolsIOEnabled =
        integrations.PROTOCOLS_IO.enabled &&
        integrations.PROTOCOLS_IO.available;
      var ownCloudEnabled =
        integrations.OWNCLOUD.enabled &&
        integrations.OWNCLOUD.available &&
        properties["ownCloud.url"] !== "";
      var nextCloudEnabled =
        integrations.NEXTCLOUD.enabled &&
        integrations.NEXTCLOUD.available &&
        properties["nextcloud.url"] !== "";
      let pyratEnabled =
        integrations.PYRAT.enabled &&
        integrations.PYRAT.available &&
        properties["pyrat.server.config"] !== "";
      const galaxyEnabled = true;
      const clustermarketEnabled =
        integrations.CLUSTERMARKET.enabled &&
        integrations.CLUSTERMARKET.available &&
        properties["clustermarket.web.url"] !== "";
      const omeroEnabled =
        integrations.OMERO.enabled &&
        integrations.OMERO.available &&
        properties["omero.api.url"] !== "";
      const joveEnabled =
        integrations.JOVE.enabled && integrations.JOVE.available;
      ...
      if (dropboxEnabled) {
        localTinymcesetup.external_plugins["dropbox"] =
          "/scripts/externalTinymcePlugins/dropbox/plugin.min.js";
        enabledFileRepositories += " dropbox";
        fileRepositoriesMenu += " optDropbox";
      }
      if (clustermarketEnabled) {
        localTinymcesetup.external_plugins["clustermarket"] =
          "/scripts/externalTinymcePlugins/clustermarket/plugin.min.js";
        localTinymcesetup.clustermarket_api_url =
          properties["clustermarket.api.url"];
        localTinymcesetup.clustermarket_web_url =
          properties["clustermarket.web.url"];
        enabledFileRepositories += " clustermarket";
        fileRepositoriesMenu += " optClustermarket";
      }
      if (omeroEnabled) {
        localTinymcesetup.external_plugins["omero"] =
          "/scripts/externalTinymcePlugins/omero/plugin.min.js";
        localTinymcesetup.omero_web_url = properties["omero.api.url"];
        enabledFileRepositories += " omero";
        fileRepositoriesMenu += " optOmero";
      }
      if (joveEnabled) {
        localTinymcesetup.external_plugins["jove"] =
          "/scripts/externalTinymcePlugins/jove/plugin.min.js";
        localTinymcesetup.jove_api_url = properties["jove.api.url"];
        enabledFileRepositories += " jove";
        fileRepositoriesMenu += " optJove";
      }
      if (boxEnabled) {
        localTinymcesetup.external_plugins["box"] =
          "/scripts/externalTinymcePlugins/box/plugin.min.js";
        enabledFileRepositories += " box";
        fileRepositoriesMenu += " optBox";
      }
      if (oneDriveEnabled) {
        localTinymcesetup.external_plugins["onedrive"] =
          "/scripts/externalTinymcePlugins/onedrive/plugin.min.js";
        enabledFileRepositories += " onedrive";
        fileRepositoriesMenu += " optOneDrive";
      }
      if (googleDriveEnabled) {
        localTinymcesetup.external_plugins["googledrive"] =
          "/scripts/externalTinymcePlugins/googledrive/plugin.min.js";
        enabledFileRepositories += " googledrive";
        fileRepositoriesMenu += " optGoogleDrive";
      }
      if (egnyteEnabled) {
        localTinymcesetup.external_plugins["egnyte"] =
          "/scripts/externalTinymcePlugins/egnyte/plugin.min.js";
        enabledFileRepositories += " egnyte";
        fileRepositoriesMenu += " optEgnyte";
      }
      if (gitHubEnabled) {
        localTinymcesetup.external_plugins["github"] =
          "/scripts/externalTinymcePlugins/github/plugin.min.js";
        enabledFileRepositories += " github";
        fileRepositoriesMenu += " optGitHub";
      }
      if (ownCloudEnabled) {
        localTinymcesetup.external_plugins["owncloud"] =
          "/scripts/externalTinymcePlugins/owncloud/plugin.min.js";
        enabledFileRepositories += " owncloud";
        fileRepositoriesMenu += " optOwnCloud";
      }
      if (nextCloudEnabled) {
        localTinymcesetup.external_plugins["nextcloud"] =
          "/scripts/externalTinymcePlugins/nextcloud/plugin.min.js";
        enabledFileRepositories += " nextcloud";
        fileRepositoriesMenu += " optNextCloud";
      }
      if (enabledFileRepositories) {
        addToToolbarIfNotPresent(
          localTinymcesetup,
          " | " + enabledFileRepositories,
        );
        addToMenuIfNotPresent(localTinymcesetup, " | " + fileRepositoriesMenu);
      }
      if (protocolsIOEnabled) {
        localTinymcesetup.external_plugins["protocols_io"] =
          "/scripts/externalTinymcePlugins/protocols_io/plugin.min.js";
        localTinymcesetup.protocols_io_access_token = getAccessToken(
          integrations.PROTOCOLS_IO,
        );
        addToToolbarIfNotPresent(localTinymcesetup, " | protocols_io");
        addToMenuIfNotPresent(localTinymcesetup, " | optProtocols_io");
      }
      if (pyratEnabled) {
        localTinymcesetup.external_plugins["pyrat"] =
          "/ui/dist/tinymcePyrat.js";
        addToToolbarIfNotPresent(localTinymcesetup, " | pyrat");
        addToMenuIfNotPresent(localTinymcesetup, " | optPyrat");
      }
      if (chemistryEnabled) {
        localTinymcesetup.external_plugins["cheminfo"] =
          "/scripts/externalTinymcePlugins/chemInfo/plugin.min.js";
      }
      if (identifiersEnabled) {
        localTinymcesetup.external_plugins["identifiers"] =
          "/ui/dist/tinymceIdentifiers.js";
        addToToolbarIfNotPresent(localTinymcesetup, " | identifiers");
        addToMenuIfNotPresent(localTinymcesetup, " | optIdentifiers");
      }
      if (galaxyEnabled) {
        localTinymcesetup.external_plugins["galaxy"] =
          "/ui/dist/tinymceGalaxy.js";
        addToToolbarIfNotPresent(localTinymcesetup, " | galaxy");
        addToMenuIfNotPresent(localTinymcesetup, " | optGalaxy");
      }
    },
  );
  ...
  requestsPromise.fail(function () {
    console.log(
      "properties, integrations or toolbar call failed - starting with default tinymce settings",
    );
  });
...
function runAfterTinymceActiveEditorInitialized(afterInitCallback) {
  if (tinyMCE.activeEditor && tinyMCE.activeEditor.initialized !== true) {
    console.log(
      "noticed tinymce editor during initialization, will wait until it's complete",
    );
...
function addToMenuIfNotPresent(localTinymcesetup, menuName) {
  if (localTinymcesetup.menu.insert.items.indexOf(menuName) === -1) {
    localTinymcesetup.menu.insert.items =
      localTinymcesetup.menu.insert.items + menuName;
  }
}
...
function uploadGalleryFile(event, ed) {
  if (event.dataTransfer) {
    ...
    $("#fromLocalComputerToGallery_" + fieldEditing).fileupload("add", {
      files: files,
    });
  }
}
  ...
  if (event.dataTransfer) {
    ...
    $.ajax({
      ...
      success: function (data) {
        let allowed_extensions = data.data,
          extension = null;

        files = Array.from(files);
        let allowed_files = files.filter((file) =>
          allowed_extensions.includes(file.name.split(".").pop()),
        );
        let removed_files = files.filter(
          (file) => !allowed_extensions.includes(file.name.split(".").pop()),
        );

        if (removed_files.length) {
          RS.confirm(
            `The following file${removed_files.length > 1 ? "s have" : " has"} an incompatible extension: <ul>${removed_files.map((f) => `<li>${f.name}</li>`)}</ul>`,
            "error",
            5000 + removed_files.length * 1000,
          );
        }
        if (allowed_files.length) {
          var fieldEditing = getFieldIdFromTextFieldId(ed.id);
          $(`#fromLocalComputerToChem_${fieldEditing}`).fileupload("add", {
            files: allowed_files,
          });
        }
      },
  ...
  if (contentSelection != "") {
    ...
    if (initAttach != -1) {
      if (contentSelection.match(/<a id=/g).length > 1) {
        ...
        var attachElement = $(
          contentSelection.substr(initAttach, endAttach + 2),
        );
...
function getAccessToken(integration) {
  ...
  $.each(integration.options, function (k, v) {
    if (k === "ACCESS_TOKEN") accessToken = "Bearer " + v;
  });
  return accessToken;
}
