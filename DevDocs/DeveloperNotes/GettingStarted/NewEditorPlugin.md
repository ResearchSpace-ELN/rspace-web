...
The back-end changes required for adding a new integration involves:
...
#### 1.1.1 Deployment properties, where applicable
Some integrations require config that varies by deployment, such as API endpoints for the customer instance of the integration. These are defined in `PropertyHolder.java`.

For your new integration, add similar properties as required:
```java
@Value("${newplugin.api.url}")
private String newPluginApiUrl;

@Value("${newplugin.api.key}")
private String newPluginApiKey;
```

These properties are then set in deployment property files:
```properties
newplugin.api.url=https://api.newplugin.com
newplugin.api.key=your-api-key-here
```

Always as a minimum add the property to the `defaultDeployment.properties` file. See [Property files docs](/DevDocs/DeveloperNotes/PropertyFiles.md) for more info.

#### 1.1.2 Sysadmin toggle
...
1. **Define the constant in `IntegrationsHandler.java` following the convention:**
   ```java
   String NEW_PLUGIN_APP_NAME = "NEW_PLUGIN";
   ```

2. **The system property is automatically constructed** by `IntegrationsHandlerImpl.getSysPropertyFromIntegrationName()`:
   ```java
   private String getSysPropertyFromIntegrationName(String name) {
     return name.toLowerCase() + ".available"; // e.g., "new_plugin.available"
   }
   ```

3. **Add a database migration** (Liquibase changeset) to create the system property entry. This should be added to the appropriate changelog file:
   ```xml
   <changeSet id="add-new-plugin-system-property" author="yourname">
     <insert tableName="SystemProperty">
       <column name="descriptor" value="new_plugin.available"/>
       <column name="type" value="BOOLEAN"/>
       <column name="defaultValue" value="false"/>
     </insert>
   </changeSet>
   ```
...
Some integrations require additional configuration (e.g. user api keys or tokens).

** App only needs to be Enabled/Disabled **
Some integrations configuration is simply a toggle for enabling/disabling the integration for the user.

These integrations use the `Preference` enum, which should be updated in the `rspace-core-model` project. 

Add the new `Preference` to the `booleanIntegrationPrefs` EnumSet in `IntegrationsHandlerImpl`:
```java
public static final EnumSet<Preference> booleanIntegrationPrefs =
    EnumSet.of(
            //... existing preferences ...
        Preference.NEW_PLUGIN);
```

** App requires individual configuration **
Most integrations use `UserAppConfig` to store other information specific to the user for that integration. 

Add your integration to the list in `IntegrationsHandlerImpl.isAppConfigIntegration()`:

```java
private boolean isAppConfigIntegration(String integrationName) {
  switch (integrationName) {
    // ... other integrations ...
    case NEW_PLUGIN_APP_NAME:
      return true;
  }
...
For integrations with configuration options, you may also need to add them to `isSingleOptionSetAppConfigIntegration()` if they only support one set of configuration options:

```java
private boolean isSingleOptionSetAppConfigIntegration(String integrationName) {
  switch (integrationName) {
    // ... other integrations ...
    case NEW_PLUGIN_APP_NAME:  // Add here if single option set
      return true;
  }
  return false;
}
...
**Exposing to Frontend (for use in section 2.1)**
The integration's state is exposed through the `IntegrationsHandler.getIntegration(User user, String integrationName)` method, which returns an `IntegrationInfo` object containing:
- `available` (boolean): Whether the sysadmin has enabled this integration
- `enabled` (boolean): Whether the user has enabled it for themselves
- `options` (Map): Any configuration options the user has set
- `displayName` (String): Human-readable name

In the frontend code (section 2.1), you access this via:
```javascript
const newPluginEnabled = integrations.NEW_PLUGIN.enabled && integrations.NEW_PLUGIN.available;
```

#### 1.1.4 Authentication
Different integrations require different authentication mechanisms. Choose the approach that matches your integration's needs:

**A. OAuth Flow**
For integrations using OAuth 2.0 (e.g., Protocols.io, Clustermarket, Figshare, Dryad, Owncloud, Nextcloud), implement:

1. **Add OAuth handling in `postProcessInfo()` method in `IntegrationsHandlerImpl`:**
   ```java
   void postProcessInfo(IntegrationInfo info, User user) {
     switch (info.getName()) {
       // ... existing cases ...
       case NEW_PLUGIN_APP_NAME:
         setOAuthConnectionStatus(info, user, NEW_PLUGIN_APP_NAME);
         return;
       default:
     }
   }
...
2. **Add token handling in `postProcessInfo()` in `IntegrationsHandlerImpl`:**
   ...
   void postProcessInfo(IntegrationInfo info, User user) {
     switch (info.getName()) {
       // ... existing cases ...
       case NEW_PLUGIN_APP_NAME:
         setSingleUserToken(info, user, NEW_PLUGIN_APP_NAME, NEW_PLUGIN_USER_TOKEN);
         return;
       default:
     }
   }
